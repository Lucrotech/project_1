...
def classify_images(images_dir, results_dic, model):
    ...
    """
    # Iterate through the results dictionary processing each pet image (filename)
    for image_file in results_dic:
        ...
        # Retrieve the real label (pet label) from the results dictionary
        real_label = results_dic[image_file][0]
        
        # Compare the pet image label and classifier label to determine if they match
        prediction = 1 if real_label in classifier_label else 0
        
        # Add the classifier label and comparison result to the results dictionary
        results_dic[image_file].append(classifier_label)
        results_dic[image_file].append(prediction)
    
    # Since results_dic is mutable, no return is needed
    return None